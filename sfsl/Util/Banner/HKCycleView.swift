...
@objc public protocol HKCycleViewProtocol {
    
    /// scrollToIndex
    ///
    /// - Parameter index: index
    @objc func hk_cycleViewDidScrollToIndex(_ index: Int)
    
    /// selectedIndex
    ///
    /// - Parameter index: index
    @objc func hk_cycleViewDidSelectedIndex(_ index: Int)
    
    // ----------------------------------- custom cell ------------------------------
    
    /// cell identifier
    /// get custom cell reuse identifier , default is 'HKCycleViewCell'
    @objc optional func hk_customCollectionViewCellIdentifier() -> String
    
    /// customCell register with class
    /// custom collectionViewCell with class
    @objc optional func hk_customCollectionViewCellClassForCycleScrollView() -> AnyClass
    
    /// customCell register with nib
    /// custom collectionViewCell with nib
    @objc optional func hk_customCollectionViewCellNibForCycleScrollView() -> UINib
    
    /// setup data for customCell
    /// setup customCell
    @objc optional func hk_setupCustomCell(_ cell: UICollectionViewCell, for index: NSInteger, cycleView: HKCycleView)
}

public class HKCycleView: UIView {
    
    /// cell identifier
    private var reuseIdentifier: String = "HKCycleViewCell"
    ...
    public var pageControlAlignment: HKPageControlAlignment = .center {
    ...
    /// delegate
    public var delegate: HKCycleViewProtocol? {
        didSet {
            if delegate != nil {
                registerCell()
            }
    ...
    //=============================================
    fileprivate var flowLayout: HKCycleLayout!
    fileprivate var collectionView: UICollectionView!
    fileprivate var placeholderImgView: UIImageView!
    fileprivate var pageControl: HKPageControl!
    ...
    fileprivate var resourceType: HKResourceType = .image
    ...
    private func addPlaceholderImgView() {
        ...
        let hCons = NSLayoutConstraint.constraints(withVisualFormat: "H:|[placeholderImgView]|",
                                                   ...
                                                   metrics: nil,
                                                   views: ["placeholderImgView": placeholderImgView as Any])
    ...
    private func registerCell() {
        if let customReuseIdentifier = delegate?.hk_customCollectionViewCellIdentifier?() {
            self.reuseIdentifier = customReuseIdentifier
        }
        if let customClass = delegate?.hk_customCollectionViewCellClassForCycleScrollView?() {
            collectionView.register(customClass, forCellWithReuseIdentifier: reuseIdentifier)
        } else if let customNib = delegate?.hk_customCollectionViewCellNibForCycleScrollView?() {
            collectionView.register(customNib, forCellWithReuseIdentifier: reuseIdentifier)
        } else {
            collectionView.register(HKCycleViewCell.self, forCellWithReuseIdentifier: reuseIdentifier)
        }
    }
    
    private func addPageControl() {
        pageControl = HKPageControl(frame: CGRect(x: 0, y: bounds.size.height - pageControlHeight, width: bounds.size.width, height: pageControlHeight))
...
extension HKCycleView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if self.delegate?.hk_setupCustomCell?(cycleCell, for: index, cycleView: self) != nil {
            // 在代码方法中配置自定义cell
            return cycleCell
        }
        
        guard let cell = cycleCell as? HKCycleViewCell else {
            return cycleCell
        }
        switch resourceType {
        case .image:
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let centerIndex = collectionView.indexPathForItem(at: centerViewPoint) {
            if indexPath.item == centerIndex.item {
                let index = indexPath.item % realDataCount
                if delegate != nil {
                    delegate?.hk_cycleViewDidSelectedIndex(index)
                } else {
...
extension HKCycleView {
    ...
    public func scrollViewDidEndScrollingAnimation(_ scrollView: UIScrollView) {
        ...
        if delegate != nil {
            delegate?.hk_cycleViewDidScrollToIndex(index)
        } else {
